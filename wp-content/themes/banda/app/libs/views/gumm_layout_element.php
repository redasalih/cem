...
abstract class GummLayoutElement extends GummObject {
    ...
    public function render($options=array()) {
        if ($this->beforeRender($options) === false) {
		...
		if ($this->getParam('enableLoadMore') === 'true' && isset($this->itemLoadableClass)) {
			echo $this->Html->nextPostsLink(__('Load More', 'gummfw'), array(
				'class' => 'load-more-link json-load-more',
				'data-loadingtext' => __('Loading', 'gummfw') . ' ...',
				'data-item-selector' => '.' . $this->itemLoadableClass,
				'data-grid-mode' => $this->getParam('postColumns'),
				'data-before' => 'parent',
				'style' => 'margin-bottom: 40px;max-width: 100%;width: auto;margin-right: 20px;margin-left: 20px;'
			));
		}
            ...
            <div class="admin-builder-element">
                ...
                <div class="template-element-settings bluebox-builder-popup">
                    ...
                    <div class="options-container builder-buttons-container">
                        <button type="button" class="btn btn-large btn-primary bb-window-save"><?php _e('Done', 'gummfw'); ?></button>
                    </div>
                </div>
...
<?php
	...
	protected function thumbnailLinkClose() {
        global $post;
            ...
            if (count($post->Media) > 0) {
                for ($i=1; $i<count($post->Media); $i++) {
                    $extraLinkAtts = array(
						...
						'rel' => 'prettyPhoto[pp-' . $this->id() . $post->ID . ']',
                        'href'  => $post->Media[$i]->permalink,
                    );
                    $outputHtml .= '<a' . $this->Html->_constructTagAttributes($extraLinkAtts) . '><img alt="' . $post->Media[$i]->post_title . '"/></a>';
                }
            }
            break;
         default:
