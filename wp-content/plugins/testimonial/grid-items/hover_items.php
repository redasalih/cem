...
if ( ! defined('ABSPATH')) exit;  // if direct access
	...
	elseif($post_format=='gallery')
	...
	else
		{
				...
				if($testimonial_hover_items_zoom_display == 'yes')
					{
						$html_hover .= '<a title="Zoom." class="zoom"></a>';
					}
				
				if($testimonial_hover_items_link_display == 'yes')
					{
						$html_hover .= '<a title="Read More." href="'.get_the_permalink(get_the_ID()).'" class="post-link"></a>';
					}								
				
				if($testimonial_hover_items_share_display == 'yes')
					{
				...
				$html .= apply_filters( 'testimonial_filter_hover', $html_hover );
